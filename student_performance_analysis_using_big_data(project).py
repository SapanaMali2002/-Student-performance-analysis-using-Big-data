# -*- coding: utf-8 -*-
"""Student-performance-analysis-using-Big-data(Project)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1F1uH7adGJHR-py82eJM6r6_zHASJb0OY
"""
...
for i in ls:
  g = sns.catplot(x=i, data=df, kind='count', height=3)
...
h = sns.barplot(x=importances[indices], y=feat_labels[indices])
...
# Use KFold with shuffle set to True
for name, model in pipelines:
    kfold = KFold(n_splits=10, shuffle=True, random_state=7)  # Set shuffle=True
    cv_results = cross_val_score(model, X_train_new, y_train, cv=kfold, scoring='neg_mean_squared_error')
    results.append(cv_results)
    names.append(name)
    msg = "%s: %f (%f)" % (name, cv_results.mean(), cv_results.std())
